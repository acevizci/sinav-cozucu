...
// ================= AI SUBJECTS / OPEN-ENDED IMPROVEMENT =================
...
if (btnAiSubjects){
  btnAiSubjects.onclick = async () => {
    try{
      ...
      // MCQ var mı? (options/choices varsa MCQ say)
      ...
      if (mcqCount === 0) {
        // OPEN-ENDED pratik: zayıf/orta yanıtları geliştirme önerileri üret
        if (typeof window.generateOpenEndedImprovementsWithGemini !== "function") {
          return showWarn({id:"AI_OE_IMPROVE_MODULE_MISSING", text:"Yanıt geliştirme modülü bulunamadı."});
        }
        await window.generateOpenEndedImprovementsWithGemini(state, {
          retryThreshold: 50,
          partialThreshold: 80,
          limit: 8
        });
      } else {
        // MCQ: konu etiketlerini tamamla
        if (typeof window.fillMissingSubjectsWithGemini !== "function") {
          return showWarn({id:"AI_SUBJECT_MODULE_MISSING"});
        }
...
safeBind("btnWrongMode", () => {
  ...
  // Açık uçlu sorular varsa meta flag'i set et (examHookOpenEnded için)
  try {
    const hasOE = (state.parsed.questions || []).some(q => q?.kind === "openEndedPro");
    if (hasOE) {
      state.parsed.meta = state.parsed.meta || {};
...
const _renderSparklineDual = (progress = []) => {
  ...
  const rows = reportItems.map((item, idx) => {
    ...
    // Açık uçlu soru mu? (yeni kayıt: kind/openEnded; eski kayıt: yourId prefix veya status)
    const isOpenEnded = q.kind === "openEndedPro"
      || (q.openEnded && typeof q.openEnded === "object")
      || String(item.yourId || "").startsWith("OPEN_ENDED")
      || item.status === "OPEN_ENDED_RETRY"
      || item.status === "OPEN_ENDED";
