...
function shouldShowQuestion(state, qN){
  ...
  // ✅ Open-ended PRO filtreleri: sadece "Boş" filtresinde anlamlı
  if (q?.kind === "openEndedPro"){
    const subs = (chosen && typeof chosen === "object") ? (chosen.subs || {}) : {};
    const hasAny = Object.values(subs).some(v => String(v||"").trim().length > 0);
    if (onlyBlank) return !hasAny;
    if (onlyWrong) return true; // anahtar yok; "yanlış" filtresinde saklamayalım
    return true;
  }
...
export function renderExam(state){
  ...
  for (const q of state.parsed.questions){
    ...
    // ✅ Open-ended PRO (şıksız) sorular
    if (q?.kind === "openEndedPro"){
      // Render a minimal question card skeleton so the injector can enhance it.
      const qDiv = document.createElement("div");
      qDiv.className = "q";
      qDiv.dataset.q = q.n;

      const subj = escapeHtml(getQuestionSubject(q));
      const qText = (q.openEnded && q.openEnded.subQuestion && q.openEnded.subQuestion.text)
        ? escapeHtml(q.openEnded.subQuestion.text).replace(/\n/g,"<br>")
        : (q.text || q.stem || "");

      qDiv.innerHTML = `
        <div class="qTop">
          <div class="qTopLeft">
            <span class="badge">Soru</span>
            <span class="badge subject-chip" id="subj-chip-${q.n}">${subj}</span>
          </div>
          <div class="qTopRight"></div>
        </div>
