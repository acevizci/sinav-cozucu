  ...
  modal.innerHTML = `
    <div class="modalCard" style="max-width:760px; width:min(760px, 92vw);">
      <div style="display:flex; align-items:center; justify-content:space-between; gap:10px;">
        <div>
          <div id="aiSubTitle" style="font-weight:700; font-size:16px;">🤖 AI ile Konuları Tamamla</div>
      ...
      <!-- ✅ Open-ended improvement: start generated practice -->
      <div id="aiSubPracticeActions" style="display:none; margin-top:14px; padding-top:12px; border-top:1px solid #2c2c2e; gap:10px; justify-content:flex-end; flex-wrap:wrap;">
        <button id="btnAiSubStartPractice" class="btn primary" style="background:linear-gradient(135deg, rgba(168,85,247,0.95) 0%, rgba(59,130,246,0.95) 100%); box-shadow:0 8px 24px rgba(59,130,246,0.18);">🧪 Bu Sorularla Pratik Başlat</button>
      </div>
    </div>
  `;
...
function _openAiSubjectModal(){
  ...
  // Başlık metni: Summary'de sınav tipine göre ayarlanabilir
  try{
    const t = modal.querySelector("#aiSubTitle");
    const wanted = window.__ACUMEN_AI_SUBJECT_TITLE;
    if (t && wanted) t.textContent = wanted;
  }catch{}
...
try { window.generateOpenEndedImprovementsWithGemini = generateOpenEndedImprovementsWithGemini; } catch {}
...
// =========================================================
// OPEN-ENDED IMPROVEMENT (Practice mode) - Yanıt geliştirme
// =========================================================
export async function generateOpenEndedImprovementsWithGemini(state, {
  retryThreshold = 50,
  partialThreshold = 80,
  limit = 8
} = {}) {
  ...
  const oe = st?.openEndedScore || window.__OPEN_ENDED_SCORE || null;

  if (!parsed || !Array.isArray(parsed.questions) || !parsed.questions.length) {
    throw appError("ERR_AI_YANIT_GELISTIRME_ICIN_SORU_BULUNAM");
  }
  if (!oe || !Array.isArray(oe.perCard) || !oe.perCard.length) {
    throw appError("ERR_AI_YANIT_GELISTIRME_ICIN_OE_SKOR_YOK");
  }

  // Candidate cards: retry + partial (recommended)
  const candidates = oe.perCard
    .filter(c => typeof c?.score === "number")
    .filter(c => c.score < partialThreshold)
    .slice(0, Math.max(1, limit));

  if (!candidates.length) {
    try { showToast?.({ kind:"info", id:"AI_OE_NO_WEAK", text:"Geliştirilecek yanıt bulunamadı." }); } catch {}
    return;
  }
  ...
  if (!apiKey) {
    apiKey = await requestApiKeyFromModal();
    if (!apiKey) throw appError("ERR_GEMINI_API_ANAHTARI_GIRILMEDI_3");
    localStorage.setItem("GEMINI_KEY", apiKey);
  }

  const modal = _openAiSubjectModal();
  _updateAiSubjectModal({ total: candidates.length, done: 0, applied: 0, suggested: 0, status: "Yanıtlar analiz ediliyor..." });
  ...
  const suggestWrap = modal.querySelector("#aiSubSuggestWrap");
  const suggestList = modal.querySelector("#aiSubSuggestList");
  ...
  const getQuestionByN = (n) => parsed.questions.find(q => (q?.n === n) || (q?.origN === n) || (String(q?.n) === String(n)));
  const getAnswerBundle = (n) => {
    const a = st?.answers?.[n] || st?.answers?.[String(n)] || null;
    if (!a || a.__type !== "open-ended") return { answerText:"", feedback:"", rubric:null, confidence:null, modelInfo:null };
    const parts = a.parts || {};
    const texts = [];
    const fbs = [];
    let rubric = null;
    ...
    for (const k of Object.keys(parts)) {
      ...
      if (g) {
        if (g.feedback) fbs.push(String(g.feedback));
        if (g.subscores && !rubric) rubric = g.subscores;
        if (typeof g.confidence === "number" && confidence == null) confidence = g.confidence;
        if (g.meta && !modelInfo) modelInfo = g.meta;
      }
    }
    return {
  ...
  const buildPrompt = (card) => {
    const q = getQuestionByN(card.n) || {};
    const ab = getAnswerBundle(card.n);
    const score = Math.round(Number(card.score) || 0);

    const rubricJson = ab.rubric ? JSON.stringify(ab.rubric).slice(0, 1600) : "";
    const answer = (ab.answerText || "").slice(0, 1800);
    const feedback = (ab.feedback || "").slice(0, 900);

    return [
      "Sen deneyimli bir eğitim koçusun ve ölçme-değerlendirme uzmanısın.",
      "Aşağıdaki açık uçlu soruya öğrencinin verdiği yanıt zayıf/orta bulundu.",
      "Amaç: Öğrencinin yanıtını geliştirmesi için somut geri bildirim ve pratik üret.",
      "",
      "KURALLAR:",
      "- Çıktıyı SADECE saf JSON ver (başka metin ekleme).",
      "- Türkçe yaz.",
      "- 'improveTips' 4-7 madde olsun (kısa, uygulanabilir).",
      "- 'guidedQuestion' tek soru olsun (yönlendirici).",
      "- 'practice' 2 adet yeni pratik soru olsun (kısa).",
      "- 'simplerExercise' 1 adet daha kolay alıştırma olsun.",
      "",
      "ÇIKTI ŞABLONU:",
      "{",
      '  "n": 12,',
      '  "score": 42,',
      '  "diagnosis": "Kısa teşhis",',
      '  "improveTips": ["...", "..."],',
      '  "guidedQuestion": "....?",',
      '  "practice": ["...", "..."],',
      '  "simplerExercise": "..."',
      "}",
      "",
      `SORU (#${q.n ?? card.n}): ${String(q.text || "").slice(0, 1500)}`,
      answer ? `ÖĞRENCİ YANITI: ${answer}` : "ÖĞRENCİ YANITI: (boş)",
      `SKOR: ${score}/100`,
      feedback ? `AI GERİBİLDİRİM: ${feedback}` : "",
      rubricJson ? `RUBRİK ALT SKORLAR (varsa): ${rubricJson}` : ""
    ].filter(Boolean).join("\n");
  };

  const parseJsonSafe = (raw) => {
    const t = String(raw || "").trim().replace(/^```(?:json)?/i, "").replace(/```\s*$/i, "").trim();
  ...
  for (const card of candidates) {
    ...
    _updateAiSubjectModal({ total: candidates.length, done, applied: 0, suggested: results.length, status: `Analiz ediliyor: #${card.n}` });

    let outText = "";
    try {
      outText = await call(buildPrompt(card));
    } catch (e) {
      outText = "";
    }

    const obj = parseJsonSafe(outText) || null;
    results.push({ card, obj, raw: outText });

    // render as we go
    if (suggestList) {
      const q = getQuestionByN(card.n) || {};
      const score = Math.round(Number(card.score) || 0);
      const lvl = score < retryThreshold ? "Zayıf" : "Orta";
      const item = document.createElement("div");
      item.style.cssText = "border:1px solid #2c2c2e; background:#111114; border-radius:14px; padding:12px;";
      const diag = obj?.diagnosis ? escapeHtml(String(obj.diagnosis)) : "—";
      const tips = Array.isArray(obj?.improveTips) ? obj.improveTips.slice(0,7).map(x=>`<li>${escapeHtml(String(x))}</li>`).join("") : "";
      const guided = obj?.guidedQuestion ? escapeHtml(String(obj.guidedQuestion)) : "";
      const pr = Array.isArray(obj?.practice) ? obj.practice.slice(0,2).map(x=>`<li>${escapeHtml(String(x))}</li>`).join("") : "";
      const simp = obj?.simplerExercise ? escapeHtml(String(obj.simplerExercise)) : "";
      item.innerHTML = `
        <div style="display:flex; align-items:center; justify-content:space-between; gap:10px;">
          <div style="font-weight:700;">#${escapeHtml(String(card.n))} • ${lvl} • ${score}/100</div>
          <button class="btn ghost" style="padding:6px 10px; font-size:12px;" data-copy="1">Kopyala</button>
        </div>
        <div style="color:#8e8e93; font-size:12px; margin-top:6px;">${escapeHtml(String(q.text||"").slice(0,180))}${String(q.text||"").length>180?"…":""}</div>
        <div style="margin-top:10px;">
          <div style="font-weight:600; margin-bottom:4px;">Teşhis</div>
          <div style="color:#d1d1d6; font-size:13px;">${diag}</div>
        </div>
        ${tips ? `<div style="margin-top:10px;"><div style="font-weight:600; margin-bottom:4px;">Geliştirme Önerileri</div><ul style="margin:0 0 0 18px; color:#d1d1d6; font-size:13px; display:flex; flex-direction:column; gap:4px;">${tips}</ul></div>` : ""}
        ${guided ? `<div style="margin-top:10px;"><div style="font-weight:600; margin-bottom:4px;">Yönlendirici Soru</div><div style="color:#d1d1d6; font-size:13px;">${guided}</div></div>` : ""}
        ${pr ? `<div style="margin-top:10px;"><div style="font-weight:600; margin-bottom:4px;">Yeni Pratik</div><ul style="margin:0 0 0 18px; color:#d1d1d6; font-size:13px; display:flex; flex-direction:column; gap:4px;">${pr}</ul></div>` : ""}
        ${simp ? `<div style="margin-top:10px;"><div style="font-weight:600; margin-bottom:4px;">Kolay Alıştırma</div><div style="color:#d1d1d6; font-size:13px;">${simp}</div></div>` : ""}
      `;
      item.querySelector('[data-copy="1"]')?.addEventListener("click", () => {
        const txt = JSON.stringify(obj || { n: card.n, score, raw: outText }, null, 2);
        try { navigator.clipboard.writeText(txt); showToast?.({ kind:"ok", id:"COPIED", text:"Kopyalandı" }); } catch {}
      });
      suggestList.appendChild(item);
    }

    done++;
    _updateAiSubjectModal({ total: candidates.length, done, applied: 0, suggested: results.length, status: `Tamamlandı: ${done}/${candidates.length}` });
  }

  _updateAiSubjectModal({ total: candidates.length, done: Math.min(done, candidates.length), applied: 0, suggested: results.length, status: cancelled ? "Durduruldu." : "Bitti." });

  // ✅ Build generated practice set from the JSON outputs (MCQ yoksa: "Yanıtlarını Geliştir")
  try {
    const practiceSet = [];
    for (const r of results) {
      ...
      const scoreBefore = Math.round(Number(card.score) || 0);
      const baseMeta = {
        generated: true,
        source: { questionN: Number(card.n) || card.n, scoreBefore },
      };

      const focus = "Yanıt geliştirme";
      const pushQ = (text, title, difficulty) => {
        const t = String(text || "").trim();
        if (!t) return;
        practiceSet.push({
          title: title || "Pratik",
          question: t,
          difficulty: difficulty || (scoreBefore < retryThreshold ? "easy" : "medium"),
          focus,
          meta: { ...baseMeta, kind: title || "practice" }
        });
      };

      // guided question
      pushQ(obj.guidedQuestion, "Yönlendirici", "medium");

      // two practice items
      if (Array.isArray(obj.practice)) {
        obj.practice.slice(0, 3).forEach((x, i) => pushQ(x, `Pratik ${i+1}`, "medium"));
      }

      // simpler exercise
      pushQ(obj.simplerExercise, "Kolay Alıştırma", "easy");
    }

    const actions = modal.querySelector('#aiSubPracticeActions');
    const btnStart = modal.querySelector('#btnAiSubStartPractice');
    if (actions && btnStart && practiceSet.length) {
      actions.style.display = 'flex';
      btnStart.disabled = false;
      btnStart.onclick = () => {
        try {
          const generator = oe?.modelInfo || results.find(x=>x?.obj)?.obj?.meta || null;
          const examId = String(parsed?.meta?.examId || parsed?.meta?.sourceId || "") || null;
          const weakCards = candidates.map(c => Number(c.n) || c.n);
          const payload = {
            title: `Gelişim Pratiği${parsed?.title ? (": " + parsed.title) : ""}`,
            generatedFrom: { examId, weakCards },
            generator,
            practiceSet,
          };
          if (typeof window.__ACUMEN_START_GENERATED_PRACTICE === 'function') {
            window.__ACUMEN_START_GENERATED_PRACTICE(payload);
            // close modal
            try { modal.style.display = 'none'; modal.setAttribute('aria-hidden','true'); } catch {}
          } else {
            showToast?.({ kind:'warn', id:'PRACTICE_START_MISSING', text:'Pratik başlatma modülü bulunamadı.' });
          }
        } catch (e) {
          console.error(e);
        }
    ...
    console.warn('practiceSet build failed', e);
  }
}
