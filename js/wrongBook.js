...
export function addOpenEndedProToWrongBookFromExam({ parsed, answersMap, thresholdPct = 50 }) {
  ...
  for (const q of qs) {
    ...
    rec.q = {
      text: q.text,
      subject: normSubject(q.subject || rec.q?.subject),
      optionsByLetter: {},
      kind: "openEndedPro",
      openEnded: q.openEnded || null,
    };
    rec.correctId = null;
    rec.yourLetter = null;
    rec.yourId = yourText ? ("OPEN_ENDED:" + yourText.slice(0, 240)) : "OPEN_ENDED";
    rec.userAnswer = yourText || null;
    rec.lastGrade = grades.length ? grades[grades.length - 1] : null;
